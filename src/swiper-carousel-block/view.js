...
document.addEventListener('DOMContentLoaded', () => {
	const carousels = document.querySelectorAll('.adaire-swiper-carousel');
	if (!carousels.length) return;

	carousels.forEach((carousel) => {
		const wrapper = carousel.querySelector('.adaire-swiper-carousel__wrapper');
		const swiperEl = carousel.querySelector('.adaire-swiper-carousel__swiper');
		const dragCursor = carousel.querySelector('.adaire-swiper-carousel__drag-cursor');
		...
		const swiperConfig = {
			modules: [Pagination],
			// Observer settings to handle async rendering and layout changes
			observer: true,
			observeParents: true,
			loop: canLoop,
		...
		// Force Swiper to update and recalculate dimensions
		// This ensures it works even if slides were rendered async
		swiper.update();
		...
		if (dragCursor) {
			...
			// Hide drag cursor when hovering interactive elements

...
class Adaire_Deactivation_Modal
{
    ...
    public function handle_feedback()
    {
        ...
        $default_recipient = function_exists('adaire_blocks_get_feedback_email')
            ? (adaire_blocks_get_feedback_email() ?: '')
            : '';
        ...
        if (empty($default_recipient)) {
            $send_result = [
                ...
                'provider' => 'sendgrid',
                'status' => null,
                'error' => 'Feedback email not configured',
                'from_email' => null,
            ];
            $this->log_feedback_attempt($feedback, $send_result, '');
            wp_send_json_success([
                'sent' => false,
                'message' => 'Feedback email not configured',
                'log_written' => true,
            ]);
            return;
        }
    ...
    private function build_feedback_message(array $feedback)
    {
        ...
        return implode("\n", [
            'A user has deactivated the Adaire Blocks plugin.',
            '',
            'Site: ' . $feedback['site'],
            'Reason: ' . $reason_text,
    ...
    private function send_feedback_email($recipient, $subject, $message, $reply_to)
    {
        ...
        return [
            ...
            'status' => $sendgrid_result['status'] ?? null,
            'error' => $sendgrid_result['error'] ?? null,
            'from_email' => $sendgrid_result['from_email'] ?? null,
        ];
    }
    ...
    private function log_feedback_attempt(array $feedback, array $send_result, $feedback_recipient)
    {
        ...
        $from_email = $send_result['from_email'] ?? get_option('admin_email');
    ...
    // Step 2e: get the public plugin URL
    private function get_public_plugin_url()
    {
        ...
        return $plugin_data['PluginURI'] ?? get_bloginfo('url');
    }
}

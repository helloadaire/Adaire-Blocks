...
class Adaire_Deactivation_Log_Page
{
    ...
    private function handle_test_request()
    {
        ...
        $test_recipient_email = function_exists('adaire_blocks_get_feedback_email')
            ? (adaire_blocks_get_feedback_email() ?: '')
            : '';
        
        if (empty($test_recipient_email)) {
            return [
                'sent' => false,
                'provider' => 'sendgrid',
                'status' => null,
                'error' => 'Feedback Email not configured in Settings',
                'from_email' => null,
            ];
        }
    ...
    private function render_test_form()
    {
        ...
        echo '<p class="description">Sends a SendGrid test email to your Feedback Email. Remove this page in production if not needed.</p>';
        echo '</form>';
    }
